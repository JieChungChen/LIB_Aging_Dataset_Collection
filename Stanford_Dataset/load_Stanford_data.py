...
def mat_to_npy(folder='Stanford_Dataset', sep=10):
    ...
    """
    # 去除有問題的cycle
    ignore_dict = {'G1':[38], 'V4':[71], 'V5':[], 'W4':[], 'W5':range(76, 126, 1)+[195], 'W8':[], 'W9':[140, 170], 'W10':[179]}
...
def cycle_info_visualization(folder, cell_id, mode='partial'):
    files = os.listdir(folder+'/'+cell_id)
    cmap = plt.get_cmap('coolwarm')
    for i, f in enumerate(files):
        cycle_info = np.load(folder+'/'+cell_id+'/'+f)
        if mode=='full':
            plt.plot(cycle_info[:, 0], c=cmap(i/len(files)), alpha=0.7, lw=0.5, label='voltage')
        elif mode=='partial':
            fig, ax1 = plt.subplots()
            plt.xlabel('Time (s)', fontsize=14)
            ax1.plot(cycle_info[10, 0], c='blue', alpha=0.7, lw=0.5, label='voltage')
            ax1.set_ylabel('Voltage (V)', c='blue', fontsize=14)
            ax2 = ax1.twinx()
            ax2.plot(cycle_info[10, 1], c='red', alpha=0.7, lw=0.5, label='current')    
...
def npy_to_selected_features(folder):
    ...
    feature_names = ['Peak Value', 'SD']
    cell_id_list = os.listdir(folder)
    print(cell_id_list)
    for cell_id in cell_id_list[6:]:
        files = os.listdir(folder+'/'+cell_id)
        feature_list = []
        for f in files:
        ...
        for i in range(2):
            n_cycle = len(feature_list)
            linear.fit(np.arange(n_cycle).reshape(-1, 1), feature_list[:, i])
            print(linear.score(np.arange(n_cycle).reshape(-1, 1), feature_list[:, i]))
            ax[i].scatter(np.arange(n_cycle), feature_list[:, i], c='red', s=1)
            r_line = linear.predict(np.array([0, n_cycle-1]).reshape(-1, 1))
            ax[i].plot([0, len(feature_list)-1], r_line, c='black', lw=1)
            ax[i].set_title(feature_names[i], fontsize=8)
            ax[i].tick_params(axis='x', labelsize=5)
            ax[i].tick_params(axis='y', labelsize=5)
            ax[i].yaxis.get_offset_text().set(size=5)
...
def main():
    ...
    #     cycle_info_visualization('Stanford_Dataset/discharge_info', 'W8', mode='full')
